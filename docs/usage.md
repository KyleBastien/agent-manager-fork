...
Pi does not include an MCP client. Its sessions reach the same workspace through the subcommands: `agent-manager --help` lists them, from `sessions`, `spawn`, `send` and `wait` to the shared task list, file reservations, terminals and the review declarations. Every subcommand except `update` acts as the session it runs in, read from the environment the launch exported; a [terminal tab](#terminal-tabs) has no such environment, so there the calling session is resolved from the tmux pane instead, and the same subcommands work from a shell you opened with `T`.
